  ...
  template: `
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      <form *ngIf="!isSubmitted" (ngSubmit)="onSubmit()" class="bg-white rounded-xl border border-border-light shadow-sm overflow-hidden">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-1.5">
              ...
              <input type="text" [(ngModel)]="formData.firstName" name="firstName" required
                class="w-full px-4 py-2.5 rounded-lg border border-gray-300 bg-surface focus:bg-white focus:ring-2 focus:ring-brand-primary/50 focus:border-brand-primary outline-none transition-all duration-200" 
                placeholder="Jean">
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-1.5">
              ...
              <select [(ngModel)]="formData.campus" name="campus"
                class="w-full px-4 py-2.5 rounded-lg border border-gray-300 bg-surface focus:bg-white focus:ring-2 focus:ring-brand-primary/50 focus:border-brand-primary outline-none transition-all duration-200 cursor-pointer">
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-border-light flex justify-end space-x-3">
          <button type="button" class="px-5 py-2.5 text-sm font-medium text-text-muted hover:text-text-main hover:bg-gray-200 rounded-lg transition-colors">
            Cancel
          </button>
          
          <button type="submit" 
            [disabled]="isProcessing"
            class="flex items-center justify-center px-6 py-2.5 text-sm font-bold text-white bg-brand-primary hover:bg-blue-700 rounded-lg shadow-md transition-all duration-200 transform hover:-translate-y-0.5 disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none">
...
export class StudentRegistrationComponent {
  ...
  onSubmit() {
    ...
    // 2. Simulate a network request to your backend (takes 1.5 seconds)
    setTimeout(() => {
      this.isProcessing = false;
      this.isSubmitted = true;
      
      // Look at your browser console to see the actual data captured!
